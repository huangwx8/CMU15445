 ...
 public:
  ...
  bool Next([[maybe_unused]] Tuple *tuple) override {
    if (plan_->IsRawInsert()) {
      return ExecuteRawInsert();
    } else {
      return ExecuteChildInsert();
    }
  }

 private:
  bool ExecuteRawInsert() {
    TableMetadata* info = exec_ctx_->GetCatalog()->GetTable(plan_->TableOid());

    for (std::vector<Value> entry : plan_->RawValues()) {
      RID rid;
      if (!info->table_->InsertTuple(Tuple(entry, &info->schema_), &rid, exec_ctx_->GetTransaction())) {
        return false;
      }
    }
    return true;
  ...
  bool ExecuteChildInsert() {
    TableMetadata* info = exec_ctx_->GetCatalog()->GetTable(plan_->TableOid());

    Tuple tuple;
    while (child_executor_->Next(&tuple)) {
      RID rid;
      if (!info->table_->InsertTuple(tuple, &rid, exec_ctx_->GetTransaction())) {
        return false;
      }
    }
    return true;
